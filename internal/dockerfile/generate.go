...
func Generate(ctx context.Context, facts map[string]interface{}) (string, error) {
	// Add build directory to template data if not present
	if _, ok := facts["build_dir"]; !ok {
		facts["build_dir"] = "."
	}
